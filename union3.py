...
def push_day(day, fname_, sname_, rname_, areasum, nterm, hareasum_, center_):
    ...
    if cur.fetchall()[0][0] == 0:
        ...
        center_new = "ST_ASText(ST_Centroid(ST_Union('"+center_old+"'::geometry,'"+center_+"'::geometry)))"
        w = d_weather(center_, day)
        cur.execute('SELECT '+center_new)
...
def d_therm(id_gar, date, region):
    sql= '''
    SELECT SUM(harea_), COUNT(area_) FROM agz_.term WHERE (fn_={id:s}) AND (rname_='{r:s}') AND (tmax_::text LIKE  '{date:s}%')
    ...
    if last_date != 0:
        while s_d <= now_date:
            ...
            sql = ''' SELECT DISTINCT ON(id_gar) id_gar, tmin_, tmax_, fname_, sname_, rname_, center_
              ...
              and '{d:s}'::timestamp + '1 day'::interval AND sname_ LIKE 'Ярос%' AND rname_  = '{r:s}' '''.format(**{'d': s_data, 'r': region})
            logging.info(sql)
            cur.execute(sql)
            ...
            for g in list_gar_one_day:
                ...
                if t[0] == 1:
                    push_day(s_data, g[3], g[4], g[5], get_area(t[1], g[0]), therm[1], therm[0], g[6])
                else:
                    push_day(s_data, g[3], g[4], g[5], get_area(t[1][0], g[0])-get_area(t[1][1], g[0]), therm[1], therm[0], g[6])
            s_d = s_d+delta
        conn.commit()
